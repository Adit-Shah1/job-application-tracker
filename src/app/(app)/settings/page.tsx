...
const PROVIDER_META: Record<
  string,
  { label: string; color: string; icon: "github" | "google" }
> = {
  github: { label: "GitHub", color: "from-zinc-700 to-zinc-900", icon: "github" },
  google: { label: "Google", color: "from-sky-500 to-emerald-500", icon: "google" },
};
...
async function ConnectedProvidersCard() {
  const session = await auth();
      ...
      <CardContent>
        {linked.length === 0 ? (
          <p className="text-sm text-zinc-500">
            No providers linked. Try signing out and back in with a
            different provider to link it.
          </p>
        ) : (
          <ul className="space-y-2">
            {linked.map((p) => (
              <li
                key={p.provider}
                className="flex items-center justify-between rounded-lg border border-zinc-200/80 bg-white/60 px-3 py-2 dark:border-zinc-800/80 dark:bg-zinc-950/60"
              >
...
function ProviderIcon({ kind }: { kind: "github" | "google" }) {
  if (kind === "github") {
    return (
      <svg
        xmlns="http://www.w3.org/2000/svg"
